...
export async function GET(req: Request) {
    ...
    const keyRecord = await prisma.apiKeyModel.findUnique({ where: { key: apiKey } });
    if (!keyRecord) return NextResponse.json({ error: "Invalid API Key" }, { status: 401 });

    const { searchParams } = new URL(req.url);
    const controlNo = searchParams.get("controlNo");
    if (!controlNo) return NextResponse.json({ error: "controlNo required" }, { status: 400 });

    try {
        const claim = await prisma.foodVoucherVendorClaim.findFirst({
            where: { claimControlNo: controlNo },
