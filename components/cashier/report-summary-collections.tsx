...
interface ReportSummaryCollectionsProps {
  reports: any[];
  userName: string;
  reportPeriod: string;
  filterType: string;
...
export function ReportSummaryCollections({ reports, userName, reportPeriod, filterType }: ReportSummaryCollectionsProps) {
  const [isOpen, setIsOpen] = useState(false);
  const qrRef = useRef<HTMLDivElement>(null);

  const collections = reports.flatMap(r => r.collections || []);

  const getReportTitle = () => {
    switch (filterType) {
      case 'week': return "WEEKLY SUMMARY COLLECTIONS REPORT";
      case 'month': return "MONTHLY SUMMARY COLLECTIONS REPORT";
      case 'quarter': return "QUARTERLY SUMMARY COLLECTIONS REPORT";
      case 'year': return "YEARLY SUMMARY COLLECTIONS REPORT";
      case 'range': return "DATE RANGE SUMMARY COLLECTIONS REPORT";
      default: return "CONSOLIDATED SUMMARY COLLECTIONS REPORT";
    }
  };
  ...
  const downloadAsPDF = async () => {
    ...
    pdf.setFontSize(11);
    pdf.setFont("helvetica", "bold");
    pdf.text(getReportTitle(), centerX, currentY, { align: "center" });
    
    currentY += 8;
    pdf.setFontSize(10);
    pdf.setFont("helvetica", "bold");
    pdf.text("Report Period:", 15, currentY);
    pdf.setFont("helvetica", "normal");
    pdf.text(reportPeriod || "All Time", 45, currentY);
    ...
    pdf.save(`Summary_Report_${filterType}_${new Date().getTime()}.pdf`);
  };
      ...
      <DialogContent className="max-w-[90vw] xl:max-w-6xl max-h-[95vh] overflow-y-auto resize overflow-auto">
        <DialogHeader className="no-print flex-row items-center justify-between">
          <div>
            ...
            <DialogDescription>
              Review the report before downloading the PDF.
            </DialogDescription>
          </div>
        ...
        <div className="w-full bg-white p-8 max-w-5xl mx-auto border shadow-sm my-4 text-black">
          ...
          <div className="text-center mb-8">
            <h2 className="text-xl font-black underline uppercase tracking-tight">{getReportTitle()}</h2>
            <p className="mt-2 text-sm">Report Period: <span className="font-bold">{reportPeriod || "All Time"}</span></p>
          </div>

          <div className="flex flex-col items-center justify-center p-4 bg-muted rounded-md mb-4 hidden">
          </div>
