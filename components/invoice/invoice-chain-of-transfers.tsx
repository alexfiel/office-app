...
interface ChainTransfer {
    ...
    transferee?: string;
    transferor?: string;
    deceasedOwner?: string; // New field from EJS modal
    heirs?: string;         // New field from EJS modal
    taxBase: number;
    totalAmountDue?: number;
    basicTaxDue?: number;  // New standardized field
        ...
        <div className="mb-10">
            ...
            <div className="space-y-4">
                ...
                {invoice.ejsChain.map((t: ChainTransfer, i: number) => {
                    const transferee = t.transferee || t.heirs || "N/A";
                    const transferor = t.transferor || t.deceasedOwner || "N/A";
                    const title = t.title || `SUBSEQUENT TRANSFER ${i + 1}`;
                    const type = t.type || "EJS";
                    const taxDue = t.totalAmountDue || t.basicTaxDue || 0;
                    ...
                    return (
                        <div key={i} className="p-5 border-2 border-slate-200 rounded-xl bg-white relative overflow-hidden shadow-sm">
                            ...
                            <div className="grid grid-cols-2 text-sm">
                                <div>
                                    <p className="text-slate-500 font-medium uppercase text-[10px]">TRANSFEREE:</p>
                                    <p className="font-semibold text-slate-900">{transferee}</p>
                                </div>
                                <div>
                                    <p className="text-slate-500 font-medium uppercase text-[10px]">TRANSFEROR:</p>
                                    <p className="font-semibold text-slate-900">{transferor}</p>
                                </div>
                            </div>
                            ...
                            <div className="mt-3 pt-3 border-t border-slate-200 grid grid-cols-2 text-sm">
                                ...
                                <p className="font-bold text-slate-900 uppercase text-[10px] text-right">
                                    TOTAL TAX DUE: {formatCurrency(taxDue)}
                                </p>
                            </div>
                        </div>
                    );
