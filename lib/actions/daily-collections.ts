...
export async function deleteDailyCollection(id: string) {
  try {
    ...
    await prisma.$transaction(async (tx) => {
      // Unlink all collections associated with this daily report
      await tx.collections.updateMany({
        where: { dailyCollectionId: id },
        data: { dailyCollectionId: null },
      });

      // Delete the daily collection
      await tx.dailyConsolidatedCollection.delete({
        where: { id },
      });
    });
    ...
    return { success: false, error: "Failed to delete daily collection." };
  }
}

export async function editDailyCollection(
  id: string,
  data: {
  ...
  try {
    ...
    await prisma.dailyConsolidatedCollection.update({
      where: { id },
      data: {
        totalDeposits: new Prisma.Decimal(data.totalDeposits),
        date: data.date,
      },
    });
    ...
    console.error("Error editing daily collection:", error);
    return { success: false, error: "Failed to edit daily collection." };
  }
}
